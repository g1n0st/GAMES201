...
wd = (10.2 * 5e-3, 10.2 * 5e-3, 15.0 * 5e-3)
...
block0 = ti.root.pointer(ti.ijk, grid_size // block_size)
block1 = block0.dense(ti.ijk, block_size)
block1.place(grid_np)
block1.dense(ti.l, MAX_GRID_NP).place(grid_p)
    ...
    for i in range(100):
        for j in range(100):
            for k in range(30):
                ...
                x[p] = [i * dx + 0.1 * 5e-3, j * dx + 0.1 * 5e-3, k * dx + 0.1 * 5e-3]
                v[p] = [0, 0, 0]
                rho[p] = rho_0
                P[p] = pressure(rho_0)
                material[p] = 1
    ...
    for i in range(150):
        for j in range(140):
            for k in range(wall_size):
                add_particle([i * dx + 0.05 * 5e-3, j * dx + 0.55 * 5e-3, k * dx + 0.05 * 5e-3], [0, 0, 0], 0)

    # left wall
    for i in range(140):
        for j in range(140):
            for k in range(wall_size):
                add_particle([k * dx + 0.05 * 5e-3, j * dx + 0.55 * 5e-3, i * dx + 0.55 * 5e-3], [0, 0, 0], 0)

    # right wall
    for i in range(140):
        for j in range(140):
            for k in range(wall_size):
                add_particle([(15 - 0.05) * 5e-3 - k * dx, j * dx + 0.55 * 5e-3, i * dx + 0.55 * 5e-3], [0, 0, 0], 0)

    # top wall
    for i in range(150):
        for j in range(140):
            for k in range(wall_size):
                add_particle([i * dx + 0.05 * 5e-3, j * dx + 0.55 * 5e-3, (15.0 - 0.05) * 5e-3 - k * dx], [0, 0, 0], 0)

    for i in range(150):
        for k in range (150):
            for j in range(wall_size):
                add_particle([i * dx + 0.05 * 5e-3, j * dx + 0.05 * 5e-3, k * dx + 0.05 * 5e-3], [0, 0, 0], 0)
    for i in range(150):
        for k in range(150):
            for j in range(wall_size):
